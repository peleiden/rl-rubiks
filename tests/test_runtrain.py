...
class TestRuntrain(MainTest):
	def test_run(self):
		...
		run_settings = {'location': location, 'rollouts': 1, 'rollout_games': 2, 'rollout_depth':2, 'batch_size':2,
				'alpha_update': 0.5, 'lr': 0.1, 'optim_fn': 'Adam', 'evaluation_interval': 2, 'is2024': True, 'arch': 'fc_small', 'analysis': True, 'nn_init': '1.2312312412e-4'}
		args = [sys.executable, run_path]
		for k, v in run_settings.items():
			args.extend([f'--{k}', str(v)])
		subprocess.check_call(args)  # Raises error on problems in call

		expected_files = ['model.pt', 'train.log',  'config.json',  'model-best.pt']
